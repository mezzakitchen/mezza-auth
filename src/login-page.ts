...
export interface LoginPageOptions {
  appName: string;               // "Mezza Sales Scorecard"
  productName?: string;          // "Mezza Scorecard" — header line above the app name
  tagline?: string;              // one sentence under the title
  logoUrl?: string;              // same-origin path to a logo image
  authBase: string;              // where the JSON endpoints live, e.g. "/auth"
  csrfMarker: string;            // value for X-Requested-With
  /** Shown when a signed-in person has no access to this application. */
  denied?: { email: string; contact?: string };
