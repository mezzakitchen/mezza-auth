...
import { deliver, loginCodeEmail } from "./email.js";
...
export function createAuth(cfg) {
    const codes = { ...DEFAULT_CODE_POLICY, ...cfg.codes };
    const limits = { ...DEFAULT_RATE_LIMITS, ...cfg.limits };
    const session = { ...DEFAULT_SESSION_POLICY, ...cfg.session };
    ...
    routes.post("/verify", async (c) => {
        ...
        if (cfg.onLogin)
            await cfg.onLogin(db, user, app, c.req.raw, env);
    ...
    if (cfg.sso) {
        ...
        routes.get("/sso", async (c) => {
            ...
            try {
                ...
                return c.text("Bad destination", 400);
            }
            const allowed = sso.allowedHosts(env).map((h) => h.toLowerCase());
            if (target.protocol !== "https:" || !allowed.includes(target.host.toLowerCase()))
                return c.text("Bad destination", 400);
            const db = cfg.db(env);
            const auth = await getSession(c);
            if (!auth) {
    ...
    return {
        ...
        clearCookieHeader: (env) => cookieHeader(cfg.cookie(env), "", 0),
        revokeUserSessions: (db, userId) => revokeUserSessions(db, userId),
    };
}
