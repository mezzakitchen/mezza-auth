...
export async function createSession(db, opts) {
    ...
    await db.prepare("INSERT INTO sessions (id, user_id, host, expires_at, last_seen_at, user_agent, ip) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(id, opts.userId, opts.host.toLowerCase(), expiresAt, nowIso(), (opts.req?.headers.get("user-agent") ?? "").slice(0, 200) || null, opts.req?.headers.get("CF-Connecting-IP") ?? null).run();
 ...
 */
export async function loadSession(db, token, host, policy) {
...
export async function destroySessionByToken(db, token) {
...
export async function sweepSessions(db, policy) {
